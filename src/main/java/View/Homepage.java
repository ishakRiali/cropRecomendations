...
public class Homepage extends JFrame{
    ...
    public Homepage(){
        ...
        try {
    InputStream inputStream = new FileInputStream("C:\\Users\\ybenk\\Documents\\NetBeansProjects\\Plantrecom\\src\\main\\java\\images\\homepage.jpg");
    BufferedImage image = ImageIO.read(inputStream);

    // Redimensionner l'image pour l'adapter à la taille de votre vue
    Image resizedImage = image.getScaledInstance(880, 430, Image.SCALE_SMOOTH);

    bgc.setIcon(new ImageIcon(resizedImage));
} catch (IOException e) {
    e.printStackTrace();
}
...
bgc.setBounds(0, 72, 880, 430);
        ...
        JLabel titre = new JLabel("Crop Prediction ");
        titre.setFont(new Font("/Fonts/Roboto-Medium.ttf", Font.BOLD, 24));
        titre.setBounds(360, 10, 500, 30);
        panel.add(titre);
